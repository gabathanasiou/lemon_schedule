...
test('element manager: date range, total days and per-day-type columns', async ({ page }) => {
  ...
  // dispatches the UI uses (SET_DAY_TYPES / UPDATE_VERSION with nonShootDates).
  const seedInfo = await page.evaluate(() => {
    ...
    const fisherId = p.castMembers?.find((m: any) => m.name === 'FISHERMAN')?.id;
    ...
    b.dispatch({
      type: 'UPDATE_VERSION', payload: {
        id: v.id,
        nonShootDates: [...(v.nonShootDates || []),
          { date: lastSectionDate, status: 'rehearsal', lists: { rehearsal: { cast: [fisherId] } } },
          { date: addDays(lastSectionDate, 1), status: 'travel', lists: { travel: { cast: [fisherId] } } },
          { date: addDays(lastSectionDate, 2), status: 'rehearsal', lists: { rehearsal: { cast: [fisherId] } } },
        ],
      },
    });
    return { fisherId };
  });
  expect(seedInfo.fisherId).toBeTruthy();
