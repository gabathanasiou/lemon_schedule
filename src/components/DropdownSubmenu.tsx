...
export default function DropdownSubmenu({ id, label, icon, width, side = 'right', children }: DropdownSubmenuProps) {
  ...
  const triggerClasses = isLight
    ...
    : isBlue
    ? `w-full text-left ${SUB_ITEM} rounded flex items-center gap-2 transition-colors active:transition-none outline-none cursor-pointer select-none text-zinc-300 justify-between hover:bg-blue-800 hover:text-white focus-visible:bg-blue-800 focus-visible:text-white active:bg-blue-700 active:text-white data-[state=open]:bg-blue-800 data-[state=open]:text-white`
  ...
  const contentClasses = isLight
    ...
    : isBlue
    ? `bg-blue-950/95 backdrop-blur-md border border-blue-800 rounded-lg shadow-xl z-[210] text-zinc-300 p-1 flex flex-col font-sans select-none max-h-[min(75vh,30rem)] overflow-y-auto min-w-0 ${width || 'w-48'}`
  ...
  const chevronColor = isLight ? 'text-zinc-400' : isBlue ? 'text-zinc-300' : 'text-zinc-500';
