...
const DayCell: React.FC<{
  ...
  const { setNodeRef: setEndRef } = useDroppable({
    id: `end-${dateKey}`,
    data: { type: 'DAY_END', date: dateKey, sectionIndex },
...
export const DaybreakCalendarTab: React.FC<{ onOpenScene?: (sceneId: string) => void; onOpenSceneInPopout?: (sceneId: string) => void }> = ({ onOpenScene, onOpenSceneInPopout }) => {
  ...
  const unscheduledRows = useMemo(() => {
    return augmentedRows.filter(r => {
      if (activeDragIds.has(r.id)) return false;
      if (!showBreaks && (r.type === 'BREAK' || r.type === 'NOTE' || r.type === 'DAYBREAK')) return false;
      return r.shootDay === null;
    }).sort((a, b) => a.order - b.order);
  }, [augmentedRows, activeDragIds, showBreaks]);
  ...
  const handleDragEnd = (e: DragEndEvent) => {
    ...
    if (lastInsertId && typeof lastInsertId === 'string' && !lastInsertId.startsWith('day-') && !lastInsertId.startsWith('end-') && !lastInsertId.startsWith('unscheduled')) {
          ...
          <div ref={calendarGridRef} className="flex-1 overflow-y-auto min-h-0 relative" style={{ touchAction: IS_COARSE ? 'pan-y pan-x' : undefined }}>
            ...
            <div className="grid grid-cols-7 border-l border-zinc-200">
               {days.map((day, idx) => {
                ...
                const sectionLabel = chronoDay ? `CHAPTER ${chronoDay}` : undefined;
                  return (
