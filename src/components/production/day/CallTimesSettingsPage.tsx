          ...
          {settings.stages.map((stage, i) => (
            <div key={stage.key} className="flex items-center gap-1.5">
              <div className="flex flex-col">
                <button type="button" disabled={readOnly || i === 0} onClick={() => moveStage(i, -1)} aria-label="Move stage earlier" className="text-zinc-400 hover:text-zinc-800 disabled:opacity-20">
                  <ArrowUp className="w-3 h-3" />
                </button>
                <button type="button" disabled={readOnly || i === settings.stages.length - 1} onClick={() => moveStage(i, 1)} aria-label="Move stage later" className="text-zinc-400 hover:text-zinc-800 disabled:opacity-20">
              ...
              {i < settings.stages.length - 1 ? (
                <TimeField value={stage.lead || ''} onChange={v => setStage(i, { lead: v || undefined })} readOnly={readOnly} placeholder="-1h" className="w-24" />
              ) : (
                <span className="text-[11px] text-zinc-400 w-24 text-center">anchor</span>
              )}
