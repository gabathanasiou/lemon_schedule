      ...
      if (cells && filteredRibbon && filteredWidths.length > 0) {
        ...
        for (let ri = 0; ri < numRows; ri++) {
          ...
          for (let ci = 0; ci < filteredRibbon[ri].cells.length; ci++) {
            ...
            const padBottom = ri === numRows - 1 ? cellPaddingV + edgePadding : cellPaddingV;
            for (let ci = 0; ci < nCols; ci++) {
              // Placeholder for columns covered by extended v-merge rowSpan
              ...
              if (coveredBySpan) {
                row.push({ content: '', styles: { fillColor: bgColor, textColor, fontSize: FONT_SIZE, halign: 'left', valign: 'middle', overflow: 'linebreak', cellPadding: { top: cellPaddingV, bottom: padBottom, left: cellPaddingH, right: cellPaddingH } } });
                continue;
              }
              let display = '';
              for (const [, wl] of wrapLines) {
              ...
              row.push({ content: display, styles: { fillColor: bgColor, textColor, fontSize: FONT_SIZE, halign: 'left', valign: 'middle', overflow: 'linebreak', cellPadding: { top: cellPaddingV, bottom: padBottom, left: cellPaddingH, right: cellPaddingH } } });
            }
          }
          bodyRows.push(row);
        }
