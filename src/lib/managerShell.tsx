...
export const DatabaseManagerView: React.FC<{
  ...
  const headerContent = (
    <>
      <span className="text-xs font-semibold text-zinc-700 mr-2">{category ? category.label : config.title}</span>
      {hasChanges && (
  ...
  const topBar = (
    <div className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-white border border-zinc-200/80 shadow-sm shrink-0">
      <span className="text-xs font-semibold text-zinc-800">{category ? category.label : config.title}</span>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-zinc-50 border-b border-zinc-200">
                      {config.fields.map(f => (
                        <th key={f.key} className={`sticky top-0 z-10 bg-zinc-50 px-3 py-2 text-left text-[10px] font-semibold text-zinc-400 uppercase tracking-wider ${f.width || ''}`}>{f.label}</th>
                      ))}
