...
import SceneCutModal from './components/script/SceneCutModal';
import { parseFDX, parseFountain, parseCSV, ImportResult, exportBreakdownCSV, exportSexFile, parseNewProjectFile, NEW_PROJECT_ACCEPT, knownDayNightPhrases } from './lib/import';
...
function AppContent() {
  ...
  const toggleSubPopout = (parentId: string, subTabId: string) => {
    ...
    if (isPopped) {
      ...
      if (parentId === 'breakdown' && brSubTab === subTabId) {
        const nextTab = ['sheet', 'script', 'elements', 'glide'].find(t => t !== subTabId && !newSet.has(t));
        if (nextTab) setBrSubTab(nextTab as any);
      } else if (parentId === 'design' && designSubTab === subTabId) {
        const nextTab = ['ribbons', 'colors', 'designer'].find(t => t !== subTabId && !newSet.has(t));
        ...
        if (nextTab) setCalendarSubTab(nextTab as any);
      }
    }
  };
  ...
  const [cutTarget, setCutTarget] = useState<{ sceneId: string; splitIndex?: number } | null>(null);
  const handleCutScene = useCallback((sceneId: string, splitIndex?: number) => setCutTarget({ sceneId, splitIndex }), []);
  const [pendingNewProjectScript, setPendingNewProjectScript] = useState<{ result: ImportResult; fileName: string } | null>(null);
  ...
  const handleNewProjectImport = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // mapping) before a project is built; projects/schedules load directly.
      if (parsed.kind === 'project') importProjectFromData(parsed.project);
      else setPendingNewProjectScript({ result: parsed.result, fileName: parsed.fileName });
    } catch (err: any) {
  ...
  // Custom report print: prefetch sun/weather for every shoot day BEFORE the
  ...
  return (
    ...
    <div className="h-screen bg-white flex flex-col text-[13px] print:bg-white print:text-black overflow-hidden">
      ...
      {cutTarget && <SceneCutModal sceneId={cutTarget.sceneId} initialSplitIndex={cutTarget.splitIndex} onClose={() => setCutTarget(null)} />}
      {pendingNewProjectScript && (
        <ImportDialog
          mode="new-project"
          initialResult={pendingNewProjectScript.result}
          initialFileName={pendingNewProjectScript.fileName}
          onCreateProject={(p) => { importProjectFromData(p); }}
          onClose={() => setPendingNewProjectScript(null)}
        />
      )}
      ...
      {poppedOutTabs.has('schedule') && popoutWindowsRef.current.get('schedule') && (
        <PopoutFrame title={`${project.title || 'Untitled'} - Schedule`} win={popoutWindowsRef.current.get('schedule')!} onClose={() => closePopout('schedule')} tabName="Schedule" projectTitle={project.title} onProjectTitleChange={v => renameProject(currentProjectId!, v, projectList.find(p => p.id === currentProjectId)?.driveFileId)}>
          <ScheduleTab onOpenScene={handleOpenScene} onOpenSceneInPopout={handleOpenSceneInPopout} onOpenDayManager={handleOpenDayManager} onPrint={() => setShowPrintDialog(true)} targetSceneId={scheduleTargetScene} onSceneTargetSeen={handleClearScheduleTarget} savedScrollTop={scheduleScrollTop} onScrollChange={setScheduleScrollTop} />
        </PopoutFrame>
      )}
      {poppedOutTabs.has('calendar') && popoutWindowsRef.current.get('calendar') && (
        <PopoutFrame title={`${project.title || 'Untitled'} - Calendar`} win={popoutWindowsRef.current.get('calendar')!} onClose={() => closePopout('calendar')} tabName="Calendar" projectTitle={project.title} onProjectTitleChange={v => renameProject(currentProjectId!, v, projectList.find(p => p.id === currentProjectId)?.driveFileId)}>
          <CalendarTab onOpenScene={handleOpenScene} onOpenSceneInPopout={handleOpenSceneInPopout} onOpenDayManager={handleOpenDayManager} />
        </PopoutFrame>
      )}
      ...
      {poppedOutSubTabs.breakdown?.has('script') && popoutSubWindowsRef.current.get('sub_breakdown_script') && (
        <SubTabPopoutFrame title={`${project.title || 'Untitled'} - Script`} win={popoutSubWindowsRef.current.get('sub_breakdown_script')!} onClose={() => closeSubPopout('breakdown', 'script')} tabName="Breakdown" subTabId="script" tabLabel="Script" projectTitle={project.title} onProjectTitleChange={v => renameProject(currentProjectId!, v, projectList.find(p => p.id === currentProjectId)?.driveFileId)} headerTarget={subHeaderTargets['sub_breakdown_script']} setHeaderTarget={el => setSubHeaderTargets(prev => ({ ...prev, sub_breakdown_script: el }))}>
          <ScriptView headerTarget={subHeaderTargets['sub_breakdown_script']} onOpenSheet={handleOpenSheet} onOpenSchedule={handleOpenScheduleAtScene} onUpdateScript={() => updateScriptFileRef.current?.click()} onCutScene={handleCutScene} />
        </SubTabPopoutFrame>
      )}
