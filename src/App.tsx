    ...
    <div className="h-screen bg-white flex flex-col text-[13px] print:bg-white print:text-black overflow-hidden">
      ...
      {poppedOutTabs.has('design') && popoutWindowsRef.current.get('design') && (
        <PopoutFrame title={`${project.title || 'Untitled'} - Design`} win={popoutWindowsRef.current.get('design')!} onClose={() => closePopout('design')} tabName="Design" projectTitle={project.title} onProjectTitleChange={v => renameProject(currentProjectId!, v, projectList.find(p => p.id === currentProjectId)?.driveFileId)} bg="bg-zinc-950">
          <DesignTab subTab={designSubTab} onSubTabChange={setDesignSubTab} onReportPrint={(design) => setReportPrint({ design, daybreak: { sections, computedRows } })} poppedOutSubTabs={poppedOutSubTabs.design || new Set()} onToggleSubPopout={(id) => toggleSubPopout('design', id)} onCloseSubPopout={(id) => closeSubPopout('design', id)} shiftHeld={shiftHeld} />
        </PopoutFrame>
      )}
      ...
      {poppedOutSubTabs.design?.has('designer') && popoutSubWindowsRef.current.get('sub_design_designer') && (
        <SubTabPopoutFrame title={`${project.title || 'Untitled'} - Reports Designer`} win={popoutSubWindowsRef.current.get('sub_design_designer')!} onClose={() => closeSubPopout('design', 'designer')} tabName="Design" subTabId="designer" tabLabel="Reports Designer" projectTitle={project.title} onProjectTitleChange={v => renameProject(currentProjectId!, v, projectList.find(p => p.id === currentProjectId)?.driveFileId)} headerTarget={subHeaderTargets['sub_design_designer']} setHeaderTarget={el => setSubHeaderTargets(prev => ({ ...prev, sub_design_designer: el }))} theme="dark" bg="bg-zinc-950">
          <ReportDesigner headerTarget={subHeaderTargets['sub_design_designer']} onPrint={(design) => setReportPrint({ design, daybreak: { sections, computedRows } })} />
        </SubTabPopoutFrame>
      )}
      ...
      {poppedOutSubTabs.production?.has('details') && popoutSubWindowsRef.current.get('sub_production_details') && (
        <SubTabPopoutFrame title={`${project.title || 'Untitled'} - Project Details`} win={popoutSubWindowsRef.current.get('sub_production_details')!} onClose={() => closeSubPopout('production', 'details')} tabName="Production" subTabId="details" tabLabel="Project Details" projectTitle={project.title} onProjectTitleChange={v => renameProject(currentProjectId!, v, projectList.find(p => p.id === currentProjectId)?.driveFileId)} headerTarget={subHeaderTargets['sub_production_details']} setHeaderTarget={el => setSubHeaderTargets(prev => ({ ...prev, sub_production_details: el }))}>
          <ProductionTab subTab="details" onSubTabChange={setProdSubTab} poppedOutSubTabs={poppedOutSubTabs.production || new Set()} onToggleSubPopout={(id) => toggleSubPopout('production', id)} onCloseSubPopout={(id) => closeSubPopout('production', id)} crewRoleTarget={prodCrewRole} onCrewRoleTargetChange={setProdCrewRole} />
        </SubTabPopoutFrame>
      )}
      ...
      {poppedOutSubTabs.production?.has('crewGlide') && popoutSubWindowsRef.current.get('sub_production_crewGlide') && (
        <SubTabPopoutFrame title={`${project.title || 'Untitled'} - Crew Glide`} win={popoutSubWindowsRef.current.get('sub_production_crewGlide')!} onClose={() => closeSubPopout('production', 'crewGlide')} tabName="Production" subTabId="crewGlide" tabLabel="Crew Glide" projectTitle={project.title} onProjectTitleChange={v => renameProject(currentProjectId!, v, projectList.find(p => p.id === currentProjectId)?.driveFileId)} headerTarget={subHeaderTargets['sub_production_crewGlide']} setHeaderTarget={el => setSubHeaderTargets(prev => ({ ...prev, sub_production_crewGlide: el }))}>
          <ProductionTab subTab="crewGlide" onSubTabChange={setProdSubTab} poppedOutSubTabs={poppedOutSubTabs.production || new Set()} onToggleSubPopout={(id) => toggleSubPopout('production', id)} onCloseSubPopout={(id) => closeSubPopout('production', id)} headerTarget={subHeaderTargets['sub_production_crewGlide']} crewRoleTarget={prodCrewRole} onCrewRoleTargetChange={setProdCrewRole} />
        </SubTabPopoutFrame>
      )}
