...
- Done: `project.dayTypes` registry (`DayTypeDef[]` = `{key,label,color?,icon?,attachable?,markable?,builtin?}` — built-ins `work|hold|travel|holiday` = Production Days/Hold/Travel/Day Off, materialized and reordered to DEFAULT on LOAD, fallback `getDayTypes`); `NonShootDate.status` is a day-type KEY (string), not a literal. Single write path `SET_DAY_TYPES` → `caseSetDayTypes` (`store/actions/reports.ts`) also prunes orphaned statuses in EVERY version — "deleting a type in use falls back to no status" in one place. Manager = the Calendar tab's **Day Types sub-tab** (`DayTypesTab`, ElementManager-style sidebar with icon/label/counts; `DayTypeModals` add/edit name/icon/color/attachable; built-ins fully locked — no edit/delete). Calendar sub-tabs (Calendar/Day Types) are poppable like other sub-tabs.
- **Production Days** (`work`, `markable: false`) = the DEFAULT state of every shooting day — never markable (excluded from the calendar context menu + TravelHoldModal dropdown via `getMarkableDayTypes`; marking it would wrongly skip the date). Its sidebar count + pane list the schedule's **production days** from the canonical `useDaybreakSections` (DAY N + date) — never re-derived; other types show their explicitly marked dates.
- **Attachments** (per-type attachable): `NonShootDate.travel`/`hold`/`castIds` folded into `NonShootDate.lists` (`Record<statusKey, Record<category, string[]>>`) by the LOAD migration — helpers in `nonShootHelpers.ts` are status-keyed; the day modal's status picker is a ui-kit dropdown + per-type list rows (cast = IDs, others = names, `'*'` = all). DOODs (tab + print): marked elements get the type's cell letter (travel→T, hold→H fixed; custom = label initial, tooltip/legend disambiguates), day headers show label + color fill, and per-type **count columns** render for in-use attachable customs — `deriveDood` cells + `DoodTotals.typeDayLists`. Calendar DayCell shows a code chip + grouped hover tooltip for custom-type attachments. Marking surfaces: calendar context menu (registry icons), toolbar H/T/DO quick tools, TravelHoldModal picker. Reports gained a `dayType` field (Days group, same shape as `dayLabel`). Known nuance (invariant, not a bug): the section date cursor skips statused dates (`computeRowData` `getDate`) so sections never sit on statused dates — a `days` repeat prints the column EMPTY in practice; the seam resolves faithfully per day.
- Note: the earlier modal-from-View-menu implementation is gone (replaced by the sub-tab; `DayTypesManagerModal`/`dayTypesManagerConfig`/ManagerShell `flat`+`rowLocked` knobs removed).
...
## 44. Linked elements — anchors with one-way element links (`[x]`)
- Done: one-way anchor-based element links. Storage = flat `project.elementLinks: ElementLink[]` (anchorCategory/anchorValue = `elementMatchId` — cast Board ID, other categories = name, name matching case-insensitive). Single source of truth `src/lib/elementLinks.ts` (`computePropagation` / `computeRemovedLinks` / `cascadeRemoval` / `applyLinkToScenes` / `addValueToField`) + the write-path seam `useLinkedEditGuard` — wired into every scene value write path: Scene Sheet `commitField`, Glide `commitEdit` (covers erase + paste, which route through it), stripboard + boneyard `updateScene` (SortableRibbon gets an `elementLinks` prop, memo-compared). Adding an anchor to a scene adds its linked elements; removing an anchor with remaining links shows a confirm first — cancel keeps the anchor (edit not applied), confirm cascades the linked values out. Link Manager modal (`LinkManagerModal.tsx`, Element Manager → Links button in header + action bar): picker rows are the shared `ElementPickerRow`/`ElementDropdown` extracted from the Color Rules row (`rules/ElementPicker.tsx`, `RuleConditionRow` refactored onto it), per-row Apply + "Apply All Links" retroactively (batch = one undo entry, scenes already containing a linked element left untouched), links dispatch immediately via `UPDATE_PROJECT` with dedupe. Covered by `e2e/linked-elements.spec.ts` (scene-sheet propagation, cancel/confirm removal semantics, manager add + retroactive apply).
