...
- Done: `project.dayTypes` registry (`DayTypeDef[]` = `{key,label,color?,icon?,attachable?,builtin?}` — built-ins hold/travel/holiday, materialized on LOAD, fallback `getDayTypes`); `NonShootDate.status` is a day-type KEY (string), not a literal. Single write path `SET_DAY_TYPES` → `caseSetDayTypes` (`store/actions/reports.ts`) also prunes orphaned statuses in EVERY version — "deleting a type in use falls back to no status" in one place. Manager = the Calendar tab's **Day Types sub-tab** (`DayTypesTab`, ElementManager-style sidebar with icon/label/usage counts + "used on" dates pane; `DayTypeModals` add/edit name/icon/color/attachable; built-in keys locked). Calendar sub-tabs (Calendar/Day Types) are poppable like other sub-tabs.
