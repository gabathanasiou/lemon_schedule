...
## 91. Production Rules — add to a section dropdown + "Other" divider (`[x]` Done)

**Done**: the Rules tab's **New Rule** control is now a dropdown to add a rule to a **specific section** (each cast member in board order, `id. name`), opening the rule editor pre-scoped to that cast (`RuleEditorPanel.preseedCastId`). A plain **New Rule…** item and a trailing **Other…** item open the editor un-scoped. The tab also renders an **"Other" divider** for rules with no real cast section (a single-cast rule whose castId no longer matches a member — deleted or free-typed — now groups under "Other" instead of a raw-id header; `grouped` checks `castMembers`, keeps `getRuleGroupKey` untouched, sorts "Other" last).

**Requested**: "when adding custom rules, can you get a drop down to add them to a specific section? if no section then add an 'other' divider."
