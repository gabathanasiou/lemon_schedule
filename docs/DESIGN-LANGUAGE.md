...
| Right-click / long-press menu | `ContextMenu` + `data-context-menu` targets | Fixed at (x,y), clamped to viewport, **light theme**; press-point-anchored morph, closes on Esc. Shares the kit menu machinery (highlight/keys/lock) — `ContextMenuSub` = `DropdownSubmenu`. Also anchors a **text selection** (Script tagging, roadmap 136: `ScriptTagMenu` — one item per element category, the highlighted text becomes the element; clicking an existing tag re-opens it with the category checked + Remove) |
