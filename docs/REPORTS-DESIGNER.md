...
- **Day call sheet collections (roadmap 99)**: contextual children of `days`, all resolved in `resolveCollection`:
  - `crewOfDay` — the day's crew (explicit `crewIds` → usual-crew template → full roster) with a resolved `crewCallTime` (per-person override → department precall resolved against the general call). `ReportCrewItem` carries stable `roleKey`/`id`.
  ...
  - `dayNotes` (scope `days`) prints the day's `daybreakMeta.note`. Wired through the collection registry (`reportBlocks.ts` labels/order/validity/contextual/identity) and `fieldsForScope` (`crewOfDay` → `crew`, `elementCallsOfDay`/`departmentCallsOfDay` own scopes, `locationsOfDay` → `locations`).
