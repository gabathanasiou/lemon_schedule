...
- **Call Sheet Edit Zone** (`callSheetEdit`, roadmap 10): the per-day editable region of a call-sheet template. In the REPORTS DESIGNER the zone is **locked** — a placeholder you place but can't add children to (`insertInto` descends only into repeat/table/relative); its dashed border renders only in the designer (`hint`), never in preview/print. A day-scoped host supplies `aux.callSheetBlocks` (from `daybreakMeta.callSheets[designId]`) to override the template zone children for one day; the same value threads through `ReportPreview`/`ReportPrint` → `useReportPaginator` → `ReportChunkPage`. Per-day editing lives in the Day Manager's full-surface **Call Sheet → Edit** (D17). For a design shaped `days` repeat → (…, `callSheetEdit`, …) it opens `CallSheetCanvas` — a **single white page** rendering the WHOLE design with the selected day's real data (read-only), with only the zone's blocks selectable/editable (palette rail + block chrome). Other shapes fall back to `ReportDesigner` **zone mode** (edits the zone block list only, commits through `zone.onChange`, palette scoped to `days`).
