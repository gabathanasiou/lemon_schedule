...
- **Call Sheet Edit Zone** (`callSheetEdit`, roadmap 10): the per-day editable region of a call-sheet template. The designer unlocks it like a repeat/table container (`insertInto` descends; canvas child zones; context-menu "Add block inside"); its dashed border renders only in the designer (`hint`), never in preview/print. A day-scoped host supplies `aux.callSheetBlocks` (from `daybreakMeta.callSheets[designId]`) to override the template zone children for one day; the same value threads through `ReportPreview`/`ReportPrint` → `useReportPaginator` → `ReportChunkPage`. Editing happens in the Day Manager's full-surface **Call Sheet → Edit** mode, which reuses `ReportDesigner` in **zone mode** (`zone` prop — edits only the zone block list, commits through `zone.onChange` instead of `UPDATE_REPORT_DESIGN`, header/footer chrome hidden, palette scoped to `days`).
