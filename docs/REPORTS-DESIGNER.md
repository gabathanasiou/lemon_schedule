...
- **Day call sheet collections (roadmap 99)**: contextual `crewOfDay` (child of `days`) resolves the day's crew (explicit `crewIds` → usual-crew template → full roster) with a resolved `crewCallTime` (per-person override → department precall); `dayNotes` (scope `days`) prints the day's `daybreakMeta.note`. `ReportCrewItem` carries stable `roleKey`/`id`. Wired through the collection registry (`reportBlocks.ts` labels/order/validity/contextual/identity) and `fieldsForScope` maps `crewOfDay` → `crew`. Remaining roadmap-99 collections: `elementCallsOfDay`, `departmentCallsOfDay`, `locationsOfDay`.
