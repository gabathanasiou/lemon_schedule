...
- Link Manager = `elements/LinkManagerModal.tsx` (Element Manager → Links, header + action bar); picker rows are the shared `rules/ElementPicker.tsx` `ElementDropdown`/`ElementPickerRow` (extracted from Color Rules' `RuleConditionRow` — extend the shared ones, never fork); per-row Apply + Apply All batch into one undo entry; `UPDATE_PROJECT` dispatches immediately with exact-duplicate dedupe.
- `notes` is not linkable; scene `set` (single-value) links only land when the field is empty.
