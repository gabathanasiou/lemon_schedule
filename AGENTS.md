...
- **Modal body rules**: wrap body in `<div className="p-6 space-y-5">`; labeled rows `flex items-center justify-between py-1` (label `text-xs text-zinc-300`, annotations `text-zinc-500`); segmented toggles `flex border border-zinc-700 rounded p-0.5` (selected `bg-white text-zinc-900`). **Footer buttons — one hero, rest ghost** (Project Manager pattern): exactly ONE hero button = the primary action, solid `bg-zinc-800 text-white text-xs font-semibold rounded-lg border border-zinc-700 hover:bg-zinc-700` (e.g. "+ New Project"); EVERY other button — Cancel, secondary actions, Import — is the ghost style `px-4 py-1.5 text-zinc-400 text-xs font-medium rounded-lg hover:bg-zinc-800 hover:text-zinc-200 transition-colors disabled:opacity-50`.
